...
(function() {
   ...
   // ==========================================
    ...
    const KLUCZ_POZYCJI = "mitel_hud_position"; // 'right' lub 'left'

    // NOWE KLUCZE DLA WYKRESU I ASYSTENTA PRZERWY
    ...
    // =================================================================
    // --- TUTAJ DODAJEMY NOWY STAN DLA ASYSTENTA PRZERWY I WYKRESU ---
    // =================================================================
    ...
    let daneGodzinowe = (function() {
        ...
        // Auto-reset: jeśli dzień się zmienił, czyścimy stare statystyki godzinowe
    ...
    function stworzPanelHUD() {
        ...
        // Dynamiczne pozycjonowanie na bazie localStorage
        ...
        // Obsługa kliknięcia w nagłówek (Zwiń / Rozwiń główny panel lub zmień pozycję)
        naglowek.addEventListener('click', (e) => {
            // Jeśli kliknięto przycisk zmiany strony, nie zwijaj panelu
            if (e.target.closest('#hud-change-side-btn')) return;
    ...
    try {
        // Zabezpieczenie 1: Czy zmienna w ogóle istnieje
        ...
        // Zabezpieczenie 2: Czy są jakieś godziny zapisane
        if (klucze.length === 0) {
        ...
        klucze.forEach(godzina => {
            ...
            // Zabezpieczenie 3: Rysowanie klocków tylko gdy ilość > 0
        ...
        // Gdyby wydarzył się jakikolwiek nieprzewidziany błąd, nie pozwalamy mu zepsuć HUD-a
    ...
    // Jeśli czyste sztuki zrównały się z celem -> Strzelamy!
    if (czyste >= cel && cel > 0) {
    ...
    // NAGŁÓWEK: Dodajemy przycisk "🗜️" do przełączania trybu minimalnego
    naglowek.innerHTML = `
    ...
    // Obsługa przycisku zmiany strony (< >)
    ...
    // Obsługa przycisku minimalizmu (🗜️ / 📺)
    ...
    // GENEROWANIE ZAWARTOŚCI W ZALEŻNOŚCI O TRYBU
    if (czyWidokMinimalny) {
        // --- WIDOK MINIMALISTYCZNY ---
        zawartosc.innerHTML = `
        ...
        // --- PEŁNY WIDOK (Zostaje tak jak był) ---
        zawartosc.innerHTML = `
        ...
        // Te Event Listenery podpinamy TYLKO w trybie pełnym, bo w minimalnym tych przycisków nie ma na ekranie
    ...
    // FIX PRZECIĄGANIA: Działa zawsze, niezależnie od wybranego trybu wielkości
            ...
            if (e.target.classList.contains('hud-copy-trigger-btn')) {
                ...
                if (snDoSkopiowania) {
                    navigator.clipboard.writeText(snDoSkopiowania).then(() => {
                        alert(`Skopiowano SN: ${snDoSkopiowania}`);
   ...
   // ==========================================
    ...
    function sprawdzZapisPrzedPrzeladowaniem() {
        // Funkcja pomocnicza zbierająca dane i zapisująca je do sesji przed przeładowaniem strony
        function przygotujZapisSesji(elementKlikniety, wymuszoneCzysteQC = false) {
            ...
            // 1. Sprawdzamy status w select (jeśli nie został wymuszony jako QC)
            if (!czyZmienionoNaQC) {
            ...
            // 3. Sprawdzanie czy to POWRÓT (na podstawie klasy 'fail' lub tekstu w wierszu)
            if (aktualnyWiersz) {
            ...
            // --- INTELIGENTNE WYKRYWANIE MODELU I SN ---
            let surowyModel = wykryjModelTelefonu(kontenerDanych);
            let wyciagnietySN = przechwycNumerSeryjny(kontenerDanych);

            // Gwarancja dla wierszy POWRÓT (czyszczenie ukrytych znaków HTML i białych znaków)
        ...
        // OBSŁUGA KLIKNIĘĆ (Tradycyjne przyciski DODAJ / Zatwierdzenia komentarzy)
        ...
        // LIVE FIX DLA POWROTÓW: Przechwytujemy zmianę statusu w select bez klikania przycisków
        document.addEventListener('change', function(e) {
            ...
            if (el.name === 'status' && el.classList.contains('status-select')) {
                ...
                // Przypisujemy aktualny wiersz do pamięci podręcznej, aby funkcja przygotujZapisSesji go widziała
                tymczasowyWierszTabeli = el.closest('tr');

                // Jeśli przestawiasz status urządzenia na QC (np. w czerwonym wierszu POWRÓT)
    ...
    // --- FUNKCJA POMOCNICZA: WYŁAPUJE MOMENT KLIKNIĘCIA W TABELI DLA WSZYSTKICH STATUSÓW ---
    function podepnijMonitorowanieStatusuLive() {
        ...
        tabelaGlowna.addEventListener('change', function(e) {
            ...
            // Zapamiętujemy wiersz przy każdej zmianie statusu w tabeli głównej
        ...
        // Czyszczenie referencji przy anulowaniu, ale dajemy lekkie opóźnienie, żeby sprawdzZapisPrzedPrzeladowaniem zdążył pobrać dane
        document.addEventListener('click', function(e) {
    ...
    function pobierzSumeDzisZHtml() {
        try {
            const elementyTla = document.querySelectorAll('.user-actions span, .user-actions div, .user-actions');
            let znalezionyTekst = "";
            for (let el of elementyTla) {
                if (el.textContent.includes("Dzisiaj:")) {
                    znalezionyTekst = el.textContent;
                    break;
                }
            }
            if (!znalezionyTekst) return null;
            const dopasowanie = znalezionyTekst.match(/Dzisiaj:\s*(\d+)/i);
            if (dopasowanie && dopasowanie[1]) {
                const wynik = parseInt(dopasowanie[1], 10);
                return isNaN(wynik) ? null : wynik;
            }
            return null;
        } catch (err) {
            return null;
    ...
    // --- FUNKCJA WERYFIKUJĄCA DUPLIKATY SERIALI W TABELI ---
    function sprawdzMultiZapisyWTabeli() {
        ...
        // Krok 1: Zliczamy wystąpienia każdego SN w tabeli na stronie
        ...
        // Krok 2: Jeśli dany SN się powtarza, delikatnie go wyróżniamy stylistycznie
        wiersze.forEach(wiersz => {
            const komorki = wiersz.getElementsByTagName('td');
            if (komorki.length < 3) return;
            const sn = komorki[2].textContent.trim();
            ...
            if (mapaSN[sn] > 1) {
                // Dodajemy ładną fioletową kropkę ostrzegawczą przy numerze seryjnym
                if (!komorki[2].querySelector('.multi-icon')) {
                    ...
                    wiersz.style.borderLeft = "3px solid #a855f7"; // Fioletowy akcent z boku wiersza
                }
            }
        });
    }
   ...
   function procesujStatePoPrzeladowaniu() {
        try {
            ...
            // Pobieramy informację o powrocie z sesji
                ...
                if (!przetworzoneTokeny.includes(pendingToken)) {
                    ...
                    // =========================================================
                        ...
                        if (czyZgloszonoJakoPowrot) {
                            ...
                            // Standardowa ścieżka sprawna L1 -> QC
                        ...
                        // Łączna liczba sztuk rośnie TYLKO przy nowej rejestracji
                        if (czySumaWzrosla) {
                            daneDzis.modele[zapamietanyModel].razem++;
                        }

                    // =========================================================
                    // CASE 3: Zwykłe dodanie
                    // =========================================================
                    } else {
                        if (czySumaWzrosla) {
                            daneDzis.modele[zapamietanyModel].razem++;
            ...
            sessionStorage.removeItem('mitel_pending_is_return'); // <-- CZYSZCZENIE NOWEJ FLAGI
    ...
    // Ta zmienna musi być widoczna dla obu funkcji, wklej ją nad funkcją wlaczPrzeciaganieHUD
    ...
    function dragMouseDown(e) {
        // Ignorujemy kliknięcia w przyciski i inne kontrolki w nagłówku
        if (e.target.closest('#hud-change-side-btn') || ['BUTTON', 'INPUT', 'SELECT'].includes(e.target.tagName)) return;

        e.preventDefault();
        czyPrzesunieto = false;

        // Dodajemy klasę blokującą automatyczne odświeżanie
        hud.classList.add('hud-state-dragging');

        // 1. Zapamiętujemy pozycję startową KURSORA myszy
        startX = e.clientX;
        startY = e.clientY;

        // 2. Zapamiętujemy dokładną pozycję startową OKNA w tej samej milisekundzie
    ...
    function elementDrag(e) {
        ...
        // 3. Obliczamy dystans, jaki myszka pokonała od punktu startu
        const deltaX = e.clientX - startX;
        const deltaY = e.clientY - startY;

        // 4. Ustawiamy pozycję: pozycja startowa okna + przebyta droga myszki
    ...
    function closeDragElement(e) {
        ...
        // Usuwamy klasę blokującą po puszczeniu przycisku myszy
        hud.classList.remove('hud-state-dragging');

        if (czyPrzesunieto) {
            e.stopPropagation();

            // Zapisujemy ostateczne współrzędne w przeglądarce
            const pozycja = {
                x: hud.offsetLeft,
                y: hud.offsetTop
            };
    ...
    if (window.confetti) return; // Jeśli już jest, to nie ładuj ponownie
    ...
    // Zabezpieczenie przed ponownym odpaleniem tego samego dnia w sesji
    if (sessionStorage.getItem('mitel_norma_celebrated') === 'yes') return;
    sessionStorage.setItem('mitel_norma_celebrated', 'yes');

    console.log("MitelHUD: Norma zdobyta! Odpalam konfetti i fajerwerki.");
    ...
    // Generowanie konfetti (spada z góry)
    for (let i = 0; i < 120; i++) {
    ...
    // Funkcja tworząca pojedynczą rakietę fajerwerku
    function stworzFajerwerk() {
        if (fajerwerki.length < 3) { // Maksymalnie 3 rakiety na ekranie na raz
            fajerwerki.push({
                x: Math.random() * (canvas.width * 0.6) + (canvas.width * 0.2), // środek ekranu
                y: canvas.height,
                targetY: Math.random() * (canvas.height * 0.4) + (canvas.height * 0.1), // wysokość wybuchu
    ...
    // Funkcja tworząca rozbłysk (iskry) po wybuchu rakiety
    ...
    // 2. GŁÓWNA PĘTLA ANIMACJI (Renderuje wszystko na jednym canvasie)
    function rysuj() {
        ctx.clearRect(0, 0, canvas.width, canvas.height);

        // --- SEKCJA A: FAJERWERKI (RAKIETY) ---
        // Losowo generuj nowe rakiety przez pierwsze 3.5 sekundy
        ...
        for (let i = fajerwerki.length - 1; i >= 0; i--) {
            ...
            // Rysowanie lecącej rakiety (śladu)
            ctx.beginPath();
            ctx.arc(f.x, f.y, 3, 0, Math.PI * 2);
            ctx.fillStyle = '#ffffff';
            ctx.fill();

            // Jeśli rakieta osiągnie cel -> Wybucha!
            if (f.y <= f.targetY) {
        ...
        // --- SEKCJA B: ISKRY Z WYBUCHU ---
        for (let i = iskryFajerwerkow.length - 1; i >= 0; i--) {
            const iskra = iskryFajerwerkow[i];
            iskra.x += iskra.vx;
            iskra.y += iskra.vy;
            iskra.vy += 0.04; // grawitacja ciągnie iskry w dół
            iskra.alpha -= iskra.fade; // iskra powoli znika
            ...
            ctx.fill();
            ctx.globalAlpha = 1; // reset przezroczystości
        }

        // --- SEKCJA C: SPADAJĄCE KONFETTI ---
        ...
        // Animacja działa przez maksymalnie 5.5 sekundy lub dopóki coś wciąż się rusza
    ...
    function init() {
        try {
            ...
            // Uruchomienie z opóźnieniem 50ms gwarantuje, że MitelHUD i jego <h2> już są na stronie
